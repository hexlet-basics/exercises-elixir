...
Важно понимать, что при обработке переданного в функцию `map` списка, создается новый список, а не мутируется исходный, так как язык является иммутабельным. Внутри переданной функции над элементом списка могут выполняться какие-угодно вычисления, однако итоговый список _всегда_ будет такого же размера, как и исходный:
