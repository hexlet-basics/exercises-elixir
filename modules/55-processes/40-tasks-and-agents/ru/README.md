...
Преимущества модуля `Task` в отличие от создания процессов напрямую через `spawn` заключаются в улучшенном выводе ошибок, а так же соответствующим API для обработки таких _задач_:
...
В функции `start_link` происходит _глобальная_ регистрация процесса-агента и его связывание с процессом из которого вызвана функция, подобно функции `spawn_link`. Про регистрацию процессов будет рассказано в следующих упражнениях.
