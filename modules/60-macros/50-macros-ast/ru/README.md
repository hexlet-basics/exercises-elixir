...
- Макросы не дают возможности глобально внедрять произвольный код, то есть необходимо напрямую в _конкретном_ модуле вызвать `require` или `import` нужного макроса;
- Использование макросов в коде _явно_, поэтому неожиданного поведения, как например, полного переопределения функции, за спиной у разработчика не происходит;
...
Но даже эти механизмы не снимают ответственность с разработчика за написанный им код, пишите макросы ответственно и вдумчиво, и только если в этом есть острая _необходимость_!
