...
Однако, такой подход начинает хуже работать с увеличением вложенности операций. Для _выпрямления_ таких вычислений используется конструкция `with`, с помощью которой описывается удачный путь вычислений (happy path) и комбинировать вызовы функций с разным форматом данных. Перепишем на `with`:
