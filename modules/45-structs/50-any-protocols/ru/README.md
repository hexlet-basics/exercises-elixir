...
Конечно, реализация получилось странной, потому что для любого типа будет возвращаться `something`, однако это учебный пример. Теперь опишем пользователя и явно _выведем_ для него протокол:
